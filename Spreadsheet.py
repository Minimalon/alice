...
class Spreadsheet:
    ...
    def share(self, shareRequestBody):
        ...
        if self.driveService is None:
            self.driveService = discovery.build('drive', 'v3', http=self.httpAuth)
    ...
    def get_last_cell_in_column(self, column):
        results = self.service.spreadsheets().values().batchGet(spreadsheetId=self.spreadsheetId,
                                                                ranges=[f'{self.sheetTitle}!{column}:{column}'],
                                                                valueRenderOption='FORMATTED_VALUE',
                                                                dateTimeRenderOption='FORMATTED_STRING').execute()
        return len(results["valueRanges"][0]["values"])

    def get_value_in_cell(self, cell):
        results = self.service.spreadsheets().values().batchGet(spreadsheetId=self.spreadsheetId,
    ...
    def get_dates(self):
        results = self.service.spreadsheets().values().batchGet(spreadsheetId=self.spreadsheetId,
                                                                ranges=[f'{self.sheetTitle}!A2:A33'],
                                                                valueRenderOption='FORMATTED_VALUE',
                                                                dateTimeRenderOption='FORMATTED_STRING').execute()
        return results["valueRanges"][0]["values"]

    def get_work_days(self, diapozone):
        results = self.service.spreadsheets().values().batchGet(spreadsheetId=self.spreadsheetId,
                                                                ranges=[f'{self.sheetTitle}!{diapozone}'],
                                                                valueRenderOption='FORMATTED_VALUE',
                                                                dateTimeRenderOption='FORMATTED_STRING').execute()
        return results["valueRanges"][0]["values"]
